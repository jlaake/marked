...
with RMark/MARK. MARK was using 7 CPUs for those timings and \verb|marked|
was using 1 CPU. The time difference was larger prior to the recent
implementation of parallel processing in MARK. As more group structure
and time-varying individual covariates are added the time difference
increases substantially. As a comparison of convergence performance,
we subtracted the -log-likelihood value of the RMark model fit from
the equivalent value from \verb|marked|. A negative value implies
a better fit (convergence) for the \verb|marked| package and vice-versa
for a positive value. With the initial runs of each package using
their default starting values there were \Sexpr{nrow(admb.crm.compare[abs(admb.crm.compare[,5])> 0.01,])},
