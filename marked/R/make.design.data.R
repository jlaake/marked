...
#' variables in the design data can be used in formulas to specify the model.
#' 
...
#' information matrix (PIM) in MARK is triangular.  RMark uses the
#' all-different formulation for PIMS by default, so the PIMs would be
...
#' variables which represent dynamic age variables of the animal's age through
#' time.  Non-age related grouping variables are added to the design data using
#' their names in \code{data}.  For example if \code{proc.example.data} is
#' defined using the first example in \code{process.data}, then the fields
#' \code{sex}, \code{initial.age.class} (in place of \code{age} in this case),
#' and \code{region} are added to the design data in addition to the
#' \code{group} variable that has 16 levels. The levels of the \code{group}
#' variable are created by pasting (concatenating) the values of the grouping
#' factor in order.  For example, M11 is sex=M, age class=1 and region=1.
#' 
...
#' 
#' \code{ddl=make.design.data(proc.example.data,
#' parameters=list(Phi=list(age.bins=c(0,0.5,1,2,8)),p=list(time.bins=c(1980,1983,1986))))}
#' 
...
#' right by default and open (non-inclusive) on the left, except for the first
#' interval which is closed on the left.  Had we used 0,1,2,8, 0 and 1 would
#' have been in a single interval.  Any value less than 1 and greater than 0
#' could be used in place of 0.5. Alternatively, the same bins could be
#' specified as:
#' 
#' \code{ddl=make.design.data(proc.example.data,
...
#' create the needed variable when simple binning isn't sufficient or when the
...
#' described in \code{make.mark.model in RMark}, there is actually a simpler
#' way to add simple functions of variables to a formula without defining them
#' in the design data.
#' 
...
#' for the superfluous parameters after is is created.  For example, let's
...
#' properly: \code{ mydata=process.data(chdata,model="POPAN",groups="cohort")
...
#' deleted from each parameter's design data.
#' 
...
#' \code{make.mark.model in RMark}, the argument \code{default.fixed} controls
...
#' missing design data to a row in the design matrix which has all 0's.  That
...
#' output when say every other cohort is missing. See \code{make.mark.model in
#' RMark} for more information about deleted design data and what this means to
#' development of model formula.
#' 
...
#' variables to create models for "Psi" are given in \code{make.mark.model in
#' RMark}.
#' 
