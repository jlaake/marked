...
#' @usage HMMLikelihood(id,x,start,m,T,fct_dmat,fct_gamma,ddl,parlist,parameters)
...
HMMLikelihood=function(id,x,start,m,T,fct_dmat,fct_gamma,ddl,parlist,parameters)
{  
	# Arguments:
	...
	# dmat: list of D matrices; one per occasion 1 to T
	# gamma: list of gamma matrices; one per occasion 1 to T
	...
	if(start[2]<T) # compute if first encounter<T 
	{
		...
		dmat=fct_dmat(id,ddl,parlist,parameters)
		gamma=fct_gamma(id,ddl,parlist,parameters)
...
loglikelihood=function(par,type,x,start,m,T,freq=1,fct_dmat,fct_gamma,ddl,parameters,debug=FALSE)
{
	...
	# They are split based on the ordering in the model list
	...
	neglnl=-sum(freq*sapply(1:nrow(x),function(id) 
						HMMLikelihood(id,x[id,],start[id,],m,T,
								fct_dmat,fct_gamma,ddl,parlist,parameters)))
	if(debug) cat("\npar = ",par," -lnl= ",neglnl)
	return(neglnl)
}
