...
#' \code{data}; however, this is dependent on the type of analysis model.  For
#' models such as "CJS", "Pradel" and others, it is simply the length of
#' \code{ch}.  Whereas, for "Burnham" and "Barker" models,the encounter history
#' contains both capture and resight/recovery values so \code{nocc} is one-half
#' the length of \code{ch}. Likewise, the number of \code{time.intervals}
#' depends on the model.  For models, such as "CJS", "Pradel" and others, the
#' number of \code{time.intervals} is \code{nocc-1}; whereas, for
#' capture&recovery(resight) models the number of \code{time.intervals} is
#' \code{nocc}. The default time interval is unit time (1) and if this is
...
#' "Burnham", "Barker")} \item{freq}{a dataframe of frequencies (same # of rows
...
#         xx=cbind(group.covariates,group=factor(1:nrow(group.covariates)),t(xmeans))
      }
      else
      {
         xmeans=mean(subset(data,select=numvar))         
         if(ncol(t(xmeans))==0)
            xx=data.frame(N=1)
         else
   ...
   if(!is.null(xx))
   {
     if(number.of.groups==1)
     {
        ...
        dummy.data=cbind(data.frame(ch=ch,freq=rep(0,length(ch))),xx[rep(1:number.of.groups,each=nocc),])
        names(dummy.data)=c("ch","freq",names(group.covariates),numvar)             
     }else
     {
